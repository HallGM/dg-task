...
def isIncremental(buffer):
    """TODO 1: Please implement a function which will:
     ...
     """ 
    def checkIncrements(accumulator, value): 
        return value if accumulator == value - 1 else False
    return (bool(reduce(checkIncrements, buffer)))
...
def decryptFile(candidates):
    ...
    """ 
    permus = list(permutations(candidates, 2)) #This function may be used to generate all permutations of candidate values.
    with open('data/encrypted_file', 'rb') as encrypted_file:
    ...
    for perm in permus:
        ...
        knownHeader = isKnownHeader(decrypted_data)
        if(knownHeader):
            print("key: " + str(key))
            print("iv: " + str(iv) + "\n")
            with open('data/decrypted_file' + knownHeader, 'wb') as decrypted_file:
